...
class Tropical:
    ...
    def tropicalize(self,tspan=None, param_values=None, epsilon=None, ignore=1, rho=3, verbose=True, stoch=False):
...
#         subs = dict((p, param_values[i]) for i, p in enumerate(self.model.parameters))
        ...
        if stoch:
            tout, tmpy=run_stochkit(self.model,self.t, n_runs=1, param_values=self.param_values, seed=None, algorithm="ssa", verbose=True)
            self.y=tmpy[0]
            self.tspan = tout[0]
        else: 
            self.tspan = tspan
...
def run_tropical(model, tspan, ep=1, parameters = None, sp_visualize = None, stoch=False):
    tr = Tropical(model)
    tr.tropicalize(tspan, parameters, epsilon=ep, stoch=stoch)
    if sp_visualize is not None:
        tr.visualization(drivers_v=sp_visualize)
    return tr.signatures, tr.get_drivers(), tr.get_passengers()
