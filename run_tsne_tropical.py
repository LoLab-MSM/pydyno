...
def dbscan_clus(x,name):
    ...
    plt.savefig('/home/oscar/tropical_project/tropical_clustering2000_euclidean_dbscan'+ name, dpi=150)
    plt.close(f)
    return pars_colors

species = pickle.load(open('/home/oscar/tropical_project/species_monomers_parameters_euclidean_2000.p', 'rb'))
...
for spp in species_ready:
	...
	specie_normalized = StandardScaler().fit_transform(species[spp][mm])
        ...
        dbscan_clus(x, str(spp)+str(mm))
	monomials_clusters[str(spp)+str(mm)] = dbscan_clus(x, str(spp)+str(mm))

pickle.dump(monomials_clusters, open('/home/oscar/tropical_project/monomial_clusters_info.p', 'wb'))
